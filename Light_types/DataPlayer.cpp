...
void DataPlayer::updateRainfall_fade( int (*pGetCol)(void), bool doEase )
{
    takeStep();
    ...
    if( doEase ) U = U*U*( 3.0f - 2.0f*U );// easing
    ...
    for( int c = 0; c < cols; ++c )
    {
        ...
        {
            int ofst_0 = (r+row0)*gridCols + col0 + c;// row above

            if( !BA.getBit( r*cols + c ) )// draw pLight[0]
            {
           //     if( drawOff ) pLt0[ ofst_0 ] = pLight[0];
                continue;
            }

            // a raindrop is at r,c
            Light Lt_0 = drawOff ? pLight[0] : pLt0[ ofst_0 ];// bkgd or existing Light
            // write to Light in each row
            pLt0[ ofst_0 ].r = U*Lt_0.r + ( 1.0f - U )*pLight[1].r;
            pLt0[ ofst_0 ].g = U*Lt_0.g + ( 1.0f - U )*pLight[1].g;
            pLt0[ ofst_0 ].b = U*Lt_0.b + ( 1.0f - U )*pLight[1].b;

            int ofst_1 = (r+1+row0)*gridCols + col0 + c;// row below
       //     Light Lt_1 = BA.getBit( (r+1)*cols + c ) ? pLight[1] : pLight[0];// current assigned state
