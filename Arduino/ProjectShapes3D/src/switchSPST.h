...
class switchSPST
{
    ...
    switchSPST( int PinID, float t_Wait, bool iClosed = false )
    { init( PinID, t_Wait, iClosed ); }
    ...
    int pollEvent()const
    {
        ...
        if( isClosed && !wasClosed ) return flipState ? -1 : 1;// just closed
        if( !isClosed && wasClosed ) return flipState ? 1 : -1;// just opened
