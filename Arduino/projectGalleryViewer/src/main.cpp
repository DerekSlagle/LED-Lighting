...
#include "LightSourceTypes/BufferSource.h"// includes LightGrid and LightSource
...
void init_panels()
{
  ...
  panel[2].set_SrcArea( 32, 8, 0, 24 );
  panel[2].pTgt0 = leds + 512;
  panel[2].rotIdx = 2;

  panel[3].set_SrcArea( 32, 8, 0, 16 );
...
bool init_SD()
{
  ...
  FileParser FP( "/SourcePlayers/Gallery_data.txt" );
...
void updateShift()
{
  if( BuffB_Src.col0 == 0 && BuffB_Src.row0 == 0 )
  {
    updateLEDS = false;
     return;// already home
  }
  ...
  {
      --BuffB_Src.col0;
      BuffA_Src.col0 = BuffB_Src.col0 - BuffB_Src.getCols();// A is left of B
  }
  else if( BuffB_Src.col0 < 0 )
  {
      ++BuffB_Src.col0;
      BuffA_Src.col0 = BuffB_Src.getCols() + BuffB_Src.col0;// A is right of B
  }

  if( BuffB_Src.row0 > 0 )
  {
      --BuffB_Src.row0;
      BuffA_Src.row0 = BuffB_Src.row0 - BuffB_Src.getRows();// A is above B
  }
  else if( BuffB_Src.row0 < 0 )
  {
      ++BuffB_Src.row0;
      BuffA_Src.row0 = BuffB_Src.getRows() + BuffB_Src.row0;// A is below B
  }
