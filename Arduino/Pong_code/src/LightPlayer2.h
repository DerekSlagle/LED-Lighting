...
class LightPlayer2
{
    ...
    uint16_t getPattLength()const;// lookup for each funcIndex
    ...
    // draw to 2 square grids in sequence when cols > gridCols
    void update_toDoubleGrid();
    ...
    // 100  user defined pattern
    bool ( *getState100 )( unsigned int n, unsigned int param ) = nullptr;
    uint16_t ( *getPatt100Length )( void ) = nullptr;
    ...
    unsigned int getNumLts()const{ return rows*cols; }
    ...
    void setDrawMode();
    void update_toSplitGrid();// called in update_toDoubleGrid() to handle split case
