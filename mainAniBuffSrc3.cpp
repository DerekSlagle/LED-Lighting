...
void LoadNextAnimation()// callback for pushButtA
{
  ...
  LoadAnimation( pFileNames[ fileIndex ].c_str() );
  updateLEDS = true;
}
...
JsonDocument jsonDocument(10*1024);
void ParseJson(String filename)
{
  ...
  float frametime = jsonDocument["frametime"].as<float>();

  if (jsonDocument["colors"].is<JsonArray>()) {
    JsonArray arr = jsonDocument["colors"];
    const size_t numColors = arr.size();
    Light *lightArray = new Light[numColors];
    for (int i = 0; i < numColors; i++) {
      lightArray[i].r = arr[i]["r"].as<uint8_t>();
    }
  }
}
  */
...
bool init_SD()
{
  ...
  */
  ...
  if( SD_good )
  {
    String mainFileName = rootFileName + "/SD_initAlt.txt";
...
void LoadAnimation( const char* fName )
{
  ...
  if( dataType == 1 )
  {
    LoadAnimation_fromColors( fName2.c_str() );
  }
  else if( dataType == 2 )
  {
    LoadAnimation_fromBytes( fName2.c_str() );
  }
  else if( dataType == 3 )
  {
    LoadAnimation_fromColorsOneFile( fName2.c_str() );
  }
}
